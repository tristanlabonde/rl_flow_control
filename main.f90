...
program cans
  ...
  use mod_output         , only: out0d,gen_alias,out1d,out1d_chan,out1d_stat,out2d,out3d_stat,out3d,write_log_output, &
  ...
  use mod_param          , only: ng,l,dl,dli, &
                                 ...
                                 is_stat, is_vpm, is_owb, is_sphere, &
                                 is_adjoint,cbcadjvel,bcadjvel,cbcadjpre,bcadjpre
...
!  added 
  use mod_blowing
  use mod_adjoint, only: adjoint_solver
!
  implicit none
  ...
  integer :: iscal,is
  !
  ! Volume Penarization by Kametani
  real(rp), target, allocatable, dimension(:,:,:) :: fls, flv
  integer :: i, j
  real(rp) :: xloc, yloc,zloc,dgr,rad,dzmax
  real(rp) :: wang, xpit, xofs, yofs
  real(rp) :: amp_shape, zd
  real(rp), parameter :: pi = 4.0_rp*atan(1.0_rp)
  !
  ! Statistics
  integer :: m
  integer, parameter :: mst = 20
  integer, parameter :: nstart_stat = 5000
  integer :: nst
  real(rp) :: dtstat,tstat
  real(rp) :: uloc, vloc, wloc, ploc, sloc
  real(rp), target, allocatable, dimension(:,:,:,:) :: st3
  real(rp), allocatable :: st3_old(:,:,:,:)
  real(rp), allocatable :: sm(:,:,:)
  real(rp) :: err_stat, err_loc
  real(rp) :: err_num, err_den, err_num_g, err_den_g
  real(rp), parameter :: tol_stat = 1.0e-2_rp
  integer, parameter :: ncheck_stat = 1000
  integer, parameter :: nmin_stat = 0
  ...
  ! VPM by kametani
  allocate(flv( 0:n(1)+1,0:n(2)+1,0:n(3)+1), &
           fls(0:n(1)+1,0:n(2)+1,0:n(3)+1))
  !Statistics
  allocate(st3(0:n(1)+1,0:n(2)+1,0:n(3)+1,mst))
  allocate(sm(0:n(1)+1,0:n(2)+1,0:n(3)+1))
  nst = 0
  dtstat = 0.0_rp
  tstat = 0.0_rp
  st3 = 0.0_rp
  sm  = 0.0_rp
  !$acc enter data copyin(st3)
  allocate(st3_old(0:n(1)+1,0:n(2)+1,0:n(3)+1,mst))
  st3_old = 0.0_rp
  !$acc enter data copyin(st3_old)
  !
  if(is_debug) then
  ...
  if(is_vpm) then
   if(is_sphere) then
    ...
    do k=1,n(3)
      do j=1,n(2)
        do i=1,n(1)
        xloc = (real(i,rp)-0.5_rp)*dl(1)
        yloc = (real(j,rp)-0.5_rp)*dl(2)
        zloc = zc(k)
!        fls(i,j,k)=rad-sqrt((xloc-3._rp)**2+(yloc-l(2)/2._rp)**2+(zloc-0._rp)**2)
        fls(i,j,k)=rad-sqrt((xloc-l(1)/2._rp)**2+(yloc-l(2)/2._rp)**2)
        fls(i,j,k)=max(fls(i,j,k),rad-sqrt((xloc)**2+(yloc)**2))
        fls(i,j,k)=max(fls(i,j,k),rad-sqrt((xloc-l(1))**2+(yloc)**2))
        fls(i,j,k)=max(fls(i,j,k),rad-sqrt((xloc)**2+(yloc-l(2))**2))
        fls(i,j,k)=max(fls(i,j,k),rad-sqrt((xloc-l(1))**2+(yloc-l(2))**2))
        end do
      end do
    end do
   endif
! OWBUMP
   if(is_owb) then
    wang = atan(5._rp*pi/12._rp)
    xpit = 2._rp/sin(pi/2._rp - wang)
    xofs = 3._rp + xpit/2._rp
    yofs = l(2)/2._rp        ! スパン中央
    amp_shape = 0.2_rp
    do k = 1,n(3)
      do j = 1,n(2)
        do i = 1,n(1)
          ...
          zloc = zc(k)
          ! j方向（スパン方向）で分岐
          if (yloc <= l(2)/2._rp) then
            if (xloc >  xofs + (yloc-yofs)*tan(wang) .and. &
            xloc <= xofs + xpit/2._rp + (yloc-yofs)*tan(wang)) then
              zd = amp_shape * sin(2._rp*pi/xpit * &
                   (xloc-xofs-(yloc-yofs)*tan(wang)))+zc(1)
            endif
          else
            if (xloc >  xofs - (yloc-yofs)*tan(wang) .and. &
            xloc <= xofs + xpit/2._rp - (yloc-yofs)*tan(wang)) then
  ...
  dzmax = 0.0_rp
  do k = 1, n(3)-1
    dzmax = max(dzmax, abs(zc(k+1)-zc(k)))
  end do
!  dgr = 2.0_rp*sqrt(dl(1)**2 + dl(2)**2)
  dgr = 2.0_rp*sqrt(dl(1)**2 + dl(2)**2 + dzmax**2)
   do k=1,n(3)
     do j=1,n(2)
       do i=1,n(1)
...
!  call reinit_fls_godunov(n,dl,dli,dzf,dzci,fls)
!  call update_flv_from_fls(n,dl,pi,fls,flv)
  !$acc enter data copyin(fls,flv)
  !
...
! blowing control (tristan labonde)
  call apply_wall_blowing(n, dl, istep, w)
!
    ...
    do irk=1,3
      ...
      do iscal=1,nscal
        ...
        call rk_scal(rkcoeff(:,irk),n,dli,l,dzci,dzfi,grid_vol_ratio_f,s%alpha,dt,is_bound,u,v,w, &
                     s%is_forced,s%scalf,s%source,s%fluxo,s%dsdtrko,s%val,s%f,flv)
      ...
      call rk(rkcoeff(:,irk),n,dli,dzci,dzfi,grid_vol_ratio_c,grid_vol_ratio_f,dt,visc,p, &
              is_forced,velf,bforce,gacc,beta,scalars,dudtrko,dvdtrko,dwdtrko,u,v,w,f,flv)
...
!
    ...
    !
    ! stack st3 for statistics 
    ! ---- accumulate statistics here ----
    if(istep >= nstart_stat) then
      dtstat = dt
      tstat = tstat + dtstat
      !$acc parallel loop collapse(3) default(present) async(1) &
      !$acc private(uloc,vloc,wloc,ploc)
      !$OMP PARALLEL DO COLLAPSE(3) DEFAULT(shared) &
      !$OMP PRIVATE(uloc,vloc,wloc,ploc)
      do k=1,n(3)
       do j=1,n(2)
         do i=1,n(1)
          uloc=u(i,j,k)
          ...
          wloc=w(i,j,k)
          ploc=p(i,j,k)
          st3(i,j,k,1) = st3(i,j,k,1) + uloc*dtstat
          st3(i,j,k,2) = st3(i,j,k,2) + vloc*dtstat
          st3(i,j,k,3) = st3(i,j,k,3) + wloc*dtstat
          st3(i,j,k,4) = st3(i,j,k,4) + ploc*dtstat
          st3(i,j,k,5) = st3(i,j,k,5) + uloc*uloc*dtstat
          st3(i,j,k,6) = st3(i,j,k,6) + vloc*vloc*dtstat
          st3(i,j,k,7) = st3(i,j,k,7) + wloc*wloc*dtstat
          st3(i,j,k,8) = st3(i,j,k,8) + 0.25_rp*(u(i,j,k)+u(i,j+1,k))*(v(i,j,k)+v(i+1,j,k))*dtstat
          st3(i,j,k,9) = st3(i,j,k,9) + 0.25_rp*(v(i,j,k)+v(i,j,k+1))*(w(i,j,k)+w(i,j+1,k))*dtstat
          st3(i,j,k,10) = st3(i,j,k,10) + 0.25_rp*(u(i,j,k)+u(i,j,k+1))*(w(i,j,k)+w(i+1,j,k))*dtstat
          st3(i,j,k,11) = st3(i,j,k,11) + ploc*ploc*dtstat
          st3(i,j,k,12) = st3(i,j,k,12) + 0.5_rp*uloc*(p(i,j,k)+p(i+1,j,k))*dtstat
          st3(i,j,k,13) = st3(i,j,k,13) + 0.5_rp*vloc*(p(i,j,k)+p(i,j+1,k))*dtstat
          st3(i,j,k,14) = st3(i,j,k,14) + 0.5_rp*wloc*(p(i,j,k)+p(i,j,k+1))*dtstat
         end do
       end do
      end do
      ! 統計量収束判定
      if(istep >= nstart_stat+nmin_stat .and. mod(istep,ncheck_stat) == 0) then
        err_num = 0.0_rp
        err_den = 0.0_rp
        !$acc parallel loop collapse(3) default(present) reduction(+:err_num,err_den) async(1)
          do k=1,n(3)
            do j=1,n(2)
              do i=1,n(1)
                err_num = err_num + (st3(i,j,k,5)/tstat - st3_old(i,j,k,5))**2
                err_den = err_den + (st3(i,j,k,5)/tstat)**2
              end do
            end do
          end do
        ! GPU reduction の終了待ち
        !$acc wait(1)
        ! MPI 全体で足し合わせる
        call MPI_ALLREDUCE(err_num,err_num_g,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
        call MPI_ALLREDUCE(err_den,err_den_g,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
        !$acc wait(1)
        ! 全MPI領域での相対誤差
        err_stat = sqrt(err_num_g/max(err_den_g,1.0e-30_rp))
        if(myid == 0) then
          print*, 'Statistics convergence: err_stat = ', err_stat
        endif
        if(err_stat < tol_stat) then
          if(myid == 0) then
            print*, 'Turbulent statistics converged.'
            print*, 'Writing final fields before stopping...'
          endif
          !$acc wait
          !$acc update self(u,v,w,p)
          do iscal=1,nscal
        ...
        do m=1,mst
          do k=1,n(3)
            do j=1,n(2)
              do i=1,n(1)
                st3_old(i,j,k,m) = st3(i,j,k,m)/tstat
              end do
            end do
          end do
        end do
...
! Adjoint loop  
  if(is_adjoint) then
   dt_adj = 0.1_rp*dt
   if(myid == 0) then
    write(*,*) 'before adjoint: dt=', dt
    write(*,*) 'before adjoint: time=', time
    write(*,*) 'adjoint time step =', dt_adj
    call flush(6)
   endif
   !$acc update device(st3)
   call adjoint_solver(n,u,v,w,p,dt_adj,nstep,dli,flv,fls,dzci,dzfi, &
                    rkcoeff,ng,dl,dzc,dzf, &
                    cbcadjpre,bcadjpre,cbcadjvel,bcadjvel, &
                    nb,is_bound, &
